...
namespace Acheve.Web.Http.Authorization
{
    public static class AuthorizationServiceExtensions
    {
        ...
        /// <returns></returns>
        public static Task<bool> AuthorizeAsync(this IAuthorizationService service, IPrincipal user, object resource, IAuthorizationRequirement requirement)
        {
            if (service == null)
            {
        ...
        public static Task<bool> AuthorizeAsync(this IAuthorizationService service, IPrincipal user, object resource, AuthorizationPolicy policy)
        {
            if (service == null)
            {
                throw new ArgumentNullException(nameof(service));
        ...
        /// <param name="policy">The policy to check against a specific context.</param>
        /// <returns><value>true</value> when the user fulfills the policy, <value>false</value> otherwise.</returns>
        public static Task<bool> AuthorizeAsync(this IAuthorizationService service, IPrincipal user, AuthorizationPolicy policy)
        {
            if (service == null)
            {
                throw new ArgumentNullException(nameof(service));
        ...
        /// <returns><value>true</value> when the user fulfills the policy, <value>false</value> otherwise.</returns>
        public static Task<bool> AuthorizeAsync(this IAuthorizationService service, IPrincipal user, string policyName)
        {
            if (service == null)
            {
                throw new ArgumentNullException(nameof(service));
